...
// AI Project Generation Function
exports.generateProjects = async (req, res) => {
  try {
    const { role, requiredSkills = [], existingSkills = [], missingSkills = [] } = req.body;

    if (!role) {
      return res.status(400).json({ error: 'Role is required' });
    }

    // If no Gemini API key or AI service is unavailable, return sample projects
    if (!genAI || !process.env.GEMINI_API_KEY) {
      const sampleProjects = getSampleProjects(role);
      return res.status(200).json({ 
        success: true, 
        projects: sampleProjects,
        note: "Using sample data - Add GEMINI_API_KEY to environment for AI-generated projects"
      });
    }
    ...
    const prompt = `
      As an AI career advisor, generate 3-5 practical project ideas for someone aiming to become a ${role}.
      
      Context:
      - Target Role: ${role}
      - Required Skills: ${requiredSkills.join(', ') || 'Not specified'}
      - User's Existing Skills: ${existingSkills.join(', ') || 'None detected'}
      - Skills to Learn: ${missingSkills.join(', ') || 'All required skills'}
      
      Generate project ideas that:
      1. Are practical and portfolio-worthy
      2. Help bridge skill gaps
      3. Are appropriate for the target role
      4. Include clear learning objectives
      5. Can be completed within 2-4 weeks
      
      For each project, provide:
      - Title
      - Brief description
      - Required skills (specific technologies/tools)
      - 2-3 learning resources (tutorials, documentation, courses)
      - Expected outcomes
    ...
    try {
      ...
      console.error('Failed to parse AI response, using fallback projects:', parseError);
      projects = getSampleProjects(role);
    }
    ...
    res.status(200).json({ 
      ...
      projects: projects.map(project => ({
        ...project,
        aiGuideLink: `/api/project-guide?title=${encodeURIComponent(project.title)}&role=${encodeURIComponent(role)}`
      }))
    });
    ...
    // Fallback to sample projects if AI fails
    const sampleProjects = getSampleProjects(req.body.role || 'Software Developer');
    res.status(200).json({ 
      success: true, 
      projects: sampleProjects,
      note: "AI generation failed, using sample projects"
    });
  }
};

// Enhanced Sample projects fallback function that handles ALL roles dynamically
function getSampleProjects(role) {
  ...
  // Comprehensive sample projects for ALL roles in ROLE_SKILL_MAP
  const samples = {
    // Development Roles
    'frontend developer': [
      {
  ...
  if (samples[roleLower]) {
    return samples[roleLower];
  }

  // Try case-insensitive matching for all roles in ROLE_SKILL_MAP
  for (const [roleKey, skills] of Object.entries(ROLE_SKILL_MAP)) {
    if (roleKey.toLowerCase() === roleLower) {
      // Generate dynamic projects based on the role's skills
      return generateDynamicProjects(role, skills);
    }
  }

  // Try partial matching for similar roles
  for (const [sampleRole, sampleProjects] of Object.entries(samples)) {
  ...
  // Try matching with ROLE_SKILL_MAP keys
  for (const [roleKey, skills] of Object.entries(ROLE_SKILL_MAP)) {
    if (roleLower.includes(roleKey.toLowerCase()) || roleKey.toLowerCase().includes(roleLower)) {
      return generateDynamicProjects(role, skills);
    }
  }

  // Ultimate fallback - generic project for any role
  return generateDynamicProjects(role, []);
}

// Helper function to generate dynamic projects for any role
function generateDynamicProjects(role, skills) {
  const coreSkills = skills.length > 0 ? skills.slice(0, 4) : ["core technologies", "problem solving", "implementation", "industry best practices"];
  
  return [
    {
      title: `${role} Portfolio Project`,
      description: `Build a comprehensive portfolio project to demonstrate ${role} skills and create a showcase for potential employers`,
      requiredSkills: coreSkills.slice(0, 3),
      resources: [
        {
      ...
      role: role,
      difficulty: "intermediate",
      aiGuideLink: `/api/project-guide?title=${encodeURIComponent(`${role} Portfolio Project`)}&role=${encodeURIComponent(role)}`
    },
    {
      title: `Real-world ${role} Challenge`,
      description: `Solve a practical problem using ${role} skills that mimics real industry challenges`,
      requiredSkills: [...coreSkills.slice(0, 3), "analytical thinking", "testing"],
      resources: [
        {
      ...
      role: role,
      difficulty: "advanced",
      aiGuideLink: `/api/project-guide?title=${encodeURIComponent(`Real-world ${role} Challenge`)}&role=${encodeURIComponent(role)}`
    },
    {
      title: `Learning ${role} Fundamentals`,
      description: `Master the fundamental concepts and tools required for ${role} through hands-on practice`,
      requiredSkills: coreSkills.slice(0, 2),
      resources: [
        {
      ...
      role: role,
      difficulty: "beginner",
      aiGuideLink: `/api/project-guide?title=${encodeURIComponent(`Learning ${role} Fundamentals`)}&role=${encodeURIComponent(role)}`
    },
    {
      title: `${role} Innovation Project`,
      description: `Explore innovative approaches and emerging trends in ${role} field`,
      requiredSkills: [...coreSkills, "innovation", "research", "trend analysis"],
      resources: [
        {
      ...
      role: role,
      difficulty: "advanced",
      aiGuideLink: `/api/project-guide?title=${encodeURIComponent(`${role} Innovation Project`)}&role=${encodeURIComponent(role)}`
    }
  ];
}